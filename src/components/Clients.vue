<template>
  <div class="clients">
    <h2 class="heading heading-main">My journey - Experience<span class="orange-color">:</span></h2>
    <swiper :options="swiperOption" ref="clientsSwiper">
      <swiper-slide class="d-flex align-items-center" v-for="(group, groupIndex) in clients" :key="groupIndex">
        <div class="slider-item d-flex">
          <div><img v-lazy="newImageClient1" /></div>
        </div>
        <div class="slider-item d-flex">
          <div><img v-lazy="newImageClient2" /></div>
        </div>
        <div class="slider-item d-flex">
          <div><img v-lazy="newImageClient3" height="225" width="225" /></div>
        </div>

         <div class="slider-item d-flex">
            <div><img v-lazy="newImageClient4" height="225" width="225" /></div>
          </div>
      </swiper-slide>
...
>>>>>>> e6769284ed48b0f358a673ac544e7fa361d88cdf
  ...
  methods: {
    ...
    getClients() {
      return this.$http.get(`wp/v2/clients?lang=${this.lang}&per_page=3`).then(response => {
